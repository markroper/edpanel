...
public class WebSSOProfileConsumerImpl extends AbstractProfileBase implements WebSSOProfileConsumer {
    ...
    public SAMLCredential processAuthenticationResponse(SAMLMessageContext context) throws SAMLException, org.opensaml.xml.security.SecurityException, ValidationException, DecryptionException {
        ...
        String additionalData = processAdditionalData(context);

        // Create the credential
        SAMLCredential cred = new SAMLCredential(nameId, subjectAssertion, context.getPeerEntityMetadata().getEntityID(), context.getRelayState(), attributes, context.getLocalEntityId(), additionalData);
                
        SAMLCredential.currentRawXMLResponse = additionalData; // Hack until I figure out why the SecurityContextHolder does not contain the right Authentication object.
        
        System.out.println(cred.getAdditionalData());
        return cred;
    }
     ...
     */
    protected String processAdditionalData(SAMLMessageContext context) throws SAMLException {
        // Expose the raw XML of the response
        org.w3c.dom.Element node = context.getInboundMessage().getDOM();
        org.w3c.dom.Document document = node.getOwnerDocument();
        org.w3c.dom.ls.DOMImplementationLS domImplLS = (org.w3c.dom.ls.DOMImplementationLS) document.getImplementation();
