...
CREATE TABLE `student` (
  ...
  FOREIGN KEY (`home_fk`) REFERENCES `address` (`address_id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `mailing_fk$student`
  FOREIGN KEY (`mailing_fk`) REFERENCES `address` (`address_id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
...
CREATE TABLE `notification_group` (
  ...
  FOREIGN KEY (`administrator_fk`)
    REFERENCES `administrator`(`administrator_user_fk`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  FOREIGN KEY (`section_fk`)
    REFERENCES `section`(`section_id`)
...
CREATE TABLE `triggered_notification` (
  ...
  FOREIGN KEY (`subject_user_fk`)
    ...
    ON UPDATE CASCADE)
ENGINE = InnoDB;
ALTER TABLE `triggered_notification` ADD UNIQUE `uniq_user$notification$triggerdate`(`user_fk`, `subject_user_fk`, `notification_fk`, `triggered_notification_date`);
ALTER TABLE `triggered_notification` ADD INDEX (`triggered_notification_active`);
...
CREATE TABLE `message_topic` (
  ...
  FOREIGN KEY (`behavior_fk`)
    REFERENCES `behavior`(`behavior_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  FOREIGN KEY (`gpa_fk`)
    REFERENCES `gpa`(`gpa_id`)
  ...
  FOREIGN KEY (`student_section_grade_fk`)
    REFERENCES `student_section_grade`(`student_section_grade_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE TABLE `message_thread` (
  ...
  FOREIGN KEY (`message_topic_fk`)
    REFERENCES `message_topic`(`message_topic_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE TABLE `message_thread_participant` (
  ...
  FOREIGN KEY (`message_thread_fk`)
    REFERENCES `message_thread`(`message_thread_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  FOREIGN KEY (`user_fk`)
    REFERENCES `user`(`user_id`)
