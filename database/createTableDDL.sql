...
CREATE TABLE `scholar_warehouse`.`goal` (
  ...
  `parent_fk` BIGINT(20) NOT NULL COMMENT 'Foreign key that could assocaite many different places depending on the. For assignment goals it points to student assignmnet id',
  `desired_value` DECIMAL(10) NOT NULL COMMENT 'The value the student is attempting to reach with this goal',
